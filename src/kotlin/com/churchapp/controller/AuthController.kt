...
class AuthController(
    ...
    private fun handleAuthError(error: AuthError): ResponseEntity<Any> = when (error) {
        is AuthError.InvalidCredentials -> ResponseEntity.status(HttpStatus.UNAUTHORIZED)
            .body(ErrorResponse(error.message, "INVALID_CREDENTIALS"))
        is AuthError.UserAlreadyExists -> ResponseEntity.status(HttpStatus.CONFLICT)
            .body(ErrorResponse(error.message, "USER_ALREADY_EXISTS"))
        is AuthError.UserNotFound -> ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body(ErrorResponse(error.message, "USER_NOT_FOUND"))
        is AuthError.ValidationError -> ResponseEntity.status(HttpStatus.BAD_REQUEST)
            .body(ErrorResponse(error.message, "VALIDATION_ERROR"))
        is AuthError.RegistrationFailed -> ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
            .body(ErrorResponse(error.message, "REGISTRATION_FAILED"))
        is AuthError.TokenGenerationFailed -> ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
            .body(ErrorResponse(error.message, "TOKEN_GENERATION_FAILED"))
        is AuthError.InvalidToken -> ResponseEntity.status(HttpStatus.UNAUTHORIZED)
